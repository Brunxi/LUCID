...
import sifi21_plus.free_energy as free_energy
import sifi21_plus.general_helpers as general_helpers

class SifiPipeline(object):
    def __init__(self, bowtie_db, query_sequences, output_directory, mode=0, sirna_size=21, mismatches=0, 
                accessibility_check=True, accessibility_window=8, strand_check=True, end_check=True, 
                end_stability_treshold=1.0, target_site_accessibility_treshold=0.1, terminal_check=True, 
                no_efficience=False):
        ...
        nombre_directorio = os.path.expanduser("~/siFi")

        # Intenta crear el directorio (ignorando si ya existe)
        try:
            os.makedirs(nombre_directorio, exist_ok=True)
        except Exception as e:
            print(f"Error al intentar crear el directorio: {e}")    
        
        self.rnaplfold_location = os.path.expanduser("~/miniconda3/envs/siFi/bin/")                                # Rnaplfold path
        self.bowtie_location = os.path.expanduser("~/miniconda3/envs/siFi/bin/")                                         # Bowtie path
        self.temp_location = os.path.expanduser("~/siFi")                         # Temp location
        ...
        for seq_record in SeqIO.parse(self.query_sequences, "fasta"):
            ...
            if main_targets == "Canceled":
                ...
                if input_data:
                    ...
                    ruta_destino = f"{self.output_directory}/{query_name}.json"
                    out_file = open(ruta_destino, "w")
                    ...
                    if json_lst:
                        ...
                        sifi_data = open(ruta_destino, "r").read()
                        table_data = general_helpers.get_table_data(ruta_destino)
                        for x in table_data:
                            print(x[0], x[1], x[2])
                        ###################################    
                        ...
                        ###################################
                        # plot = show_plot.DrawPlot(self.sibrna_size, query_sequence, sifi_data, None, None,
                    ...
                    # nombre_archivo = "output_sifi"
                    # directorio_destino ="/home/lucia_jimenez/sifi21INTA/"
                    ...
                    #     json.dump(temp_json_file[1], archivo_json)
    ...
    def data_to_json(self, query_name, input_data, no_target, lunp_data, main_targets):
        ...
        for data_split in input_data:
            ...
            if strand == '+':
                ...
                lunp_data_xmer = lunp_data[int(sirna_name.split('sirna')[1])-1, :].astype(np.float64).tolist()[self.accessibility_window]
    ...
    def get_main_target(self):
        """Identifies main targets based on the input FASTA sequence names."""
        if not self.bowtie_data_l:
            return None
        
        # Get query names from the input FASTA
        query_names = []
        for seq_record in SeqIO.parse(self.query_sequences, "fasta"):
            query_names.append(seq_record.id)
        
        # Get all hit names that match query names
        main_targets = []
        for hit in self.bowtie_data_l:
            hit_name = hit[2]
            if any(query_name in hit_name for query_name in query_names):
                if hit_name not in main_targets:
                    main_targets.append(hit_name)
        
        return main_targets if main_targets else None
    ...
    def run_rnaplfold(self, query_name, sequence_file):
        ...
        prc = subprocess.Popen(['RNAplfold', '-W', '%d'%self.winsize,'-L', '%d'%self.span, 
                            '-u', '%d'%self.sirna_size, '-T', '%.2f'%self.temperature], 
                            stdin=subprocess.PIPE, stdout=prc_stdout, cwd=cwd)
    ...
    def free_energy_dangling_ends(self, sirna_sequence, sirna_sequence_n2):
        """Calculate the free energy of a sequence with dangling ends"""
        # Sense5_MFE
        sense_five_seq = sirna_sequence[self.sirna_start_position:self.end_nucleotides]
        ...
        sense5_MFE_enegery = free_energy.calculate_free_energy(
            sense_five_seq, 
            check=True, 
            strict=True, 
            c_seq=sense_c_seq[::-1], 
        ...
        anti_sense5_MFE_enegery = free_energy.calculate_free_energy(
            antisense_five_seq, 
            check=True, 
            strict=True, 
            c_seq=antisense_c_seq[::-1], 
            shift=1
        )
